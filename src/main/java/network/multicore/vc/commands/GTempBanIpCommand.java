...
public class GTempBanIpCommand extends AbstractCommand {
    ...
    private int execute(CommandSource src, String targetNameIp, String duration, String reason) {
        ...
        if (targets.isEmpty()) {
            ...
            ModerationUtils.broadcast(ip, src, null, end, reason != null ? reason : messages.get("no-reason"), silent, console, Permission.PUNISHMENT_RECEIVE_BAN, "ban-ip");
        } else {
            for (User target : targets) {
                ...
                ModerationUtils.broadcast(target.getUsername(), src, null, end, reason != null ? reason : messages.get("no-reason"), silent, console, Permission.PUNISHMENT_RECEIVE_BAN, "ban-ip");
            }
        }
