...
public class TempBanCommand extends AbstractCommand {
    ...
    private int execute(CommandSource src, String targetName, String serverName, String duration, String reason) {
        ...
        Date end;
        try {
            end = PunishmentUtils.parseDurationString(duration);
                ...
                connection.connect().whenComplete((result, throwable) -> {
                    if (throwable != null || !result.isSuccessful()) {
                        target.disconnect(Text.deserialize(messages.getAndReplace("moderation.disconnect.ban",
                                "player", targetName,
                                "staff", console ? messages.get("console") : staff != null ? staff.getUsername() : messages.get("console"),
                                "server", server.getServerInfo().getName(),
                                "duration", PunishmentUtils.getDurationString(end),
                                "reason", finalReason != null ? finalReason : messages.get("no-reason")
                        )));
                    } else {
        ...
        broadcast(targetName, src, server, end, reason != null ? reason : messages.get("no-reason"), silent, console);

        return COMMAND_SUCCESS;
    }

    private void broadcast(String player, CommandSource staff, RegisteredServer server, Date end, String reason, boolean silent, boolean console) {
        String broadcast = messages.getAndReplace("moderation.broadcast.ban",
                "player", player,
        ...
        if (config.getBoolean("moderation.broadcast-server-punishment-only-in-server", false)) {
            if (!silent) {
                server.getPlayersConnected()
                        .stream()
                        .filter(p -> p.hasPermission(Permission.PUNISHMENT_RECEIVE_BAN.get()) && !p.getUsername().equalsIgnoreCase(player))
            ...
            if (!silent) {
                ...
                proxy.getAllPlayers()
                        .stream()
                        .filter(p -> p.hasPermission(Permission.PUNISHMENT_RECEIVE_BAN.get()) &&
                                p.hasPermission(Permission.PUNISHMENT_RECEIVE_SILENT.get()) &&
                                !p.getUsername().equalsIgnoreCase(player))
                        .forEach(p -> Text.send(broadcast, p));
            }
