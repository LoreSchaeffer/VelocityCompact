...
public class BanIpCommand extends AbstractCommand {
    ...
    private int execute(CommandSource src, String targetNameIp, String serverName, String reason) {
        ...
        if (targets.isEmpty()) {
            ...
            ModerationUtils.broadcast(ip, src, server, null, reason != null ? reason : messages.get("no-reason"), silent, console, Permission.PUNISHMENT_RECEIVE_BAN, "ban-ip");
        } else {
            for (User target : targets) {
                ...
                ModerationUtils.broadcast(target.getUsername(), src, server, null, reason != null ? reason : messages.get("no-reason"), silent, console, Permission.PUNISHMENT_RECEIVE_BAN, "ban-ip");
            }
        }
